...
from carsharing_common.database.mongodb import create_mongo_client, get_mongo_database
...
from app.consumers.car_commands_consumer import register_handlers
...
async def lifespan(app: FastAPI):
    global session_factory, mongo_db, redis_service, kafka_producer
    ...
    mongo_db = get_mongo_database(mongo_client, settings.mongo_db)
    await mongo_db.car_locations.create_index([("location", "2dsphere")])

    redis_service = RedisService(settings.redis_url)
    await redis_service.connect()

    kafka_producer = KafkaProducerService(
        settings.kafka_bootstrap_servers, client_id="car-service"
    )
